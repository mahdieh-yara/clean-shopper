...
| `size` | `'sm' \| 'md'` | — | Controls pill size; defaults to `'md'`. `sm` = compact, used inside ProductCard. `md` = standard, used on the product detail page. |

### Visual Structure

```
Pill shape: inline-flex items-center rounded-full font-semibold uppercase
            text-inverse (white text)

Icon + label layout:
  [icon]  [LABEL]   ← icon is aria-hidden, label is visible text
  gap-sp-1

Level → bg color mapping:
  clean   → bg-success  → icon ✓  → label "Clean"
  caution → bg-warning  → icon ⚠  → label "Caution"
  avoid   → bg-error    → icon ✕  → label "Avoid"

Size → dimension mapping:
  sm  →  px-sp-2 py-sp-1  text-caption   (used inside ProductCard)
  md  →  px-sp-3 py-sp-2  text-small     (used on product detail page)
```

### States

**Default (`md`):** Colored pill at standard size, white icon + uppercase label. Used on the product detail page where the badge has more breathing room.

**`sm` size:** Same colors, icon, and label at compact padding. Used inside `ProductCard` where vertical space is limited.

No hover, loading, or error states — the badge is always read-only.

### Usage Rules

- **Use `sm`** inside ProductCard (top-right, `shrink-0` to resist flex compression).
- **Use `md`** on the product detail page where the rating is a primary piece of information.
- **Always pair** the icon and text label together — never rely on color alone to communicate the safety level.
- **Do not use** as a standalone status indicator for anything other than product safety.
- **Do not** remove the text label or icon — both are required for accessibility.
